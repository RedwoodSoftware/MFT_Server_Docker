...
<#-- @ftlvariable name="metrics" type="com.jscape.inet.mft.audit.reporting.metricses.TopExternalDownloads" -->

<#assign external_filenames=metrics.filenames?chunk(50)?first![] />

<div title="${i18n.text("Reports.Metrics.title.topExternalDownloads", [external_filenames?size])}" data-options="collapsed:false">
    <table id="top-external-downloads-grid" role="grid" aria-label="top-external-downloads" style="height:310px"></table>
    <#if external_filenames?size != 0>
        <div id="top-external-downloads" style="width:900px;height:350px"></div>
    </#if>
</div>

<script type="text/javascript"${csp_nonce_attr}>
    $(document).ready(function() {
        $("#top-external-downloads-grid").propertygrid({
            height: 300,
            sortName: "value",
            sortOrder: "desc",
            columns: [[
    ...
    <#if external_filenames?size != 0>
        $.plot($("#top-external-downloads").on("plothover", function (e, pos, item) {
                   if (item) {
