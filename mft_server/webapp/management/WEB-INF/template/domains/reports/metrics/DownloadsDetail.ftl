...
<#-- @ftlvariable name="metrics" type="com.jscape.inet.mft.audit.reporting.metricses.DownloadsDetail" -->

<div title="${i18n.text("Reports.Metrics.title.downloadsDetail", [metrics.entries?size])}" data-options="collapsed:false">
    <table id="downloads-detail-grid" role="grid" aria-label="downloads-detail" style="height:310px"></table>
</div>
<script type="text/javascript"${csp_nonce_attr}>
    $(document).ready(function() {
        $("#downloads-detail-grid").datagrid({
            remoteSort: false,
            singleSelect: false,
            rownumbers: true,
            ...
            columns: [[
                ...
                {field: "downloads", title: "${i18n.text('Reports.Metrics.column.totalBytes')?js_string}", width: "12%", align: "right"},
