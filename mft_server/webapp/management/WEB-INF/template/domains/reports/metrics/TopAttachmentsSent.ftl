...
<#-- @ftlvariable name="metrics" type="com.jscape.inet.mft.audit.reporting.metricses.TopAttachmentsSent" -->

<#assign attachment_filenames=metrics.filenames?chunk(50)?first![] />

<div title="${i18n.text("Reports.Metrics.title.topEmailAttachmentsSent", [attachment_filenames?size])}" data-options="collapsed:false">
    <table id="top-attachments-sent-grid" role="grid" aria-label="top-attachments-sent" style="height:310px"></table>
    <#if attachment_filenames?size != 0>
        <div id="top-attachements-sent" style="width:900px;height:350px"></div>
    </#if>
</div>

<script type="text/javascript"${csp_nonce_attr}>
    $(document).ready(function() {
        $("#top-attachments-sent-grid").propertygrid({
            height: 300,
            sortName: "value",
            sortOrder: "desc",
            columns: [[
    ...
    <#if attachment_filenames?size != 0>
        $.plot($("#top-attachements-sent").on("plothover", function (e, pos, item) {
                   if (item) {
