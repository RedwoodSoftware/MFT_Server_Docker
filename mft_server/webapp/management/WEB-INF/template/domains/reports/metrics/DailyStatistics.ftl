...
<#-- @ftlvariable name="metrics" type="com.jscape.inet.mft.audit.reporting.metricses.DailyStatistics" -->

<#assign periods=0..metrics.innerPeriods-1 />

<div title="${i18n.text("Reports.Metrics.title.dailyStatistics")}" data-options="collapsed:false">
    <table role="grid" aria-label="daily-statistics" class="easyui-datagrid" style="height:310px">
        <thead>
        <tr>
            <th rowspan="2" data-options="field:'day',width:50"><@i18n.message key="Reports.Metrics.column.day" /></th>
    ...
    <div id="daily-statistics" style="margin:2em">
...
<script type="text/javascript"${csp_nonce_attr}>
    $(document).ready(function() {
        ...
        var container = $("#daily-statistics");
        var placeholder = container.find("div[role=presentation]");
        var buttons = container.find("div[role=toolbar] a[role=button]").linkbutton({toggle: true, onClick: dailyStatisticsPlot});
