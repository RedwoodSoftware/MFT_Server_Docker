...
set /p monitor="Enter monitor name:"
...
echo.
echo Login success. Proceeding to run the monitor ...
echo.
...
:: Run monitor
curl.exe -s -k -b cookie1.txt -w "%%{http_code}\n" -o NUL -H "x-js-at: !csrftoken1!" -H "X-Requested-With: XMLHttpRequest" -H "accept: application/json" -H "Content-type:application/json" -X POST %proto%://%jscapehost%:%jscapemanagmentport%/rest/management/directory-monitors/%domain%/%monitor%/run > response.txt

set /p runStatsResponse=<response.txt 
if !runStatsResponse! EQU %server204% (
echo.
echo Monitor run success ..
echo.
)
if !runStatsResponse! EQU %server404% (echo Response Code : Monitor run failed .. Invalid monitor name / domain name)
) else (echo. 
