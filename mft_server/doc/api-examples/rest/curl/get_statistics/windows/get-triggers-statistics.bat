...
:: Get Domain Stats
curl.exe -s -k -b cookie1.txt -w "%%{http_code}\n" -o triggerstats.txt -H "x-js-at: !csrftoken1!" -H "X-Requested-With: XMLHttpRequest" -H "accept: application/json" -H "Content-type:application/json" -X GET %proto%://%jscapehost%:%jscapemanagmentport%/rest/management/server/statistics/triggers > response.txt

set /p triggerStatsResponse=<response.txt 
if !triggerStatsResponse! EQU %server200% (
echo See the statistics below .. 
echo.
echo.
echo.
type triggerstats.txt
echo.
echo.
echo.
echo.
echo Response is saved to file triggerstats.txt
echo.)
if !triggerStatsResponse! EQU %server404% (echo Response Code : 404 unable to get the statistics..)
) else (echo. 
