...
echo #####################################################################
echo This example demonstrates how to remove a domain using the server API.
echo #####################################################################
...
set /p domain="Enter domain name to delete:"
...
echo.
echo Login success. Proceeding to delete domain ...
echo.
...
:: Delete domain
for /F %%G in ('curl.exe -s -k -b cookie1.txt -w "%%{http_code}\n" -o NUL -H "x-js-at: !csrftoken1!" -H "X-Requested-With: XMLHttpRequest" -H "accept: application/json" -H "Content-type:application/json" -X DELETE %proto%://%jscapehost%:%jscapemanagmentport%/rest/management/domains/%domain%
') do set delDomainResponse=%%G
if !delDomainResponse! EQU %server204% (echo Domain removed successfully..)
if !delDomainResponse! EQU %server404% (echo Response Code : 404 remove domain failed, Invalid domain name..)
) else (echo. 
