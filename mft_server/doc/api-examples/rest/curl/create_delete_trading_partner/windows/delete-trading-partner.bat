...
echo ###############################################################################
echo This example demonstrates how to delete a trading partner using the server API.
echo ###############################################################################
...
echo.
echo Login success. Proceeding to remove the trading partner ...
echo.
...
:: Check if trading partner exists
for /F %%h in ('curl.exe -s -k -b cookie1.txt -w "%%{http_code}\n" -o NUL -H "x-js-at: !csrftoken1!" -H "X-Requested-With: XMLHttpRequest" -H "accept: application/json" -H "Content-type:application/json" %proto%://%jscapehost%:%jscapemanagmentport%/rest/management/trading-partners/%domain%/%tpName%
') do set isTpAvailableResponse=%%h

if !isTpAvailableResponse! EQU %server404% (echo Response code 404. Trading partner deletion failed .. Invalid partner or domain name..)else (
:: Remove trading partner
for /F %%G in ('curl.exe -s -k -b cookie1.txt -w "%%{http_code}\n" -o NUL -H "x-js-at: !csrftoken1!" -H "X-Requested-With: XMLHttpRequest" -H "accept: application/json" -H "Content-type:application/json" -X DELETE %proto%://%jscapehost%:%jscapemanagmentport%/rest/management/trading-partners/%domain%/%tpName%
') do set delDirMonResponse=%%G

if !delDirMonResponse! EQU %server204% (echo Trading partner deleted successfully..)
if !delDirMonResponse! EQU %server404% (echo Trading partner deletion failed..)
)
)else (
echo. 
