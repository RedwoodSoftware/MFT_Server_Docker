...
set /p remotePath="Enter directory to list: :"
...
:: List Directory
curl.exe -s -k -b cookie.txt -w "%%{http_code}\n" -o listdir.txt -H "x-js-at: !csrftokenResponse!" -H "X-Requested-With: XMLHttpRequest" -H "accept: */*" -H "Content-type:application/json" %proto%://%jscapehost%:%jscapeclientport%/rest/files/list/?path=%remotePath% > response.txt

set /p dirListResponse=<response.txt

if !dirListResponse! EQU %server200% (
echo Directory Listed successfully.. 
type listdir.txt
echo. )
if !dirListResponse! EQU %server404% (echo Response Code : 404 Directory listing failed, Recheck the parameters..)
) else (echo. 
echo Login failed. Re-check user and password)
)

:: Logout from the server
curl.exe -s -k -b cooke.txt -o NUL -X POST %proto%://%jscapehost%:%jscapeclientport%/rest/logout

:: delete cookie
DEL cookie.txt
IF exist response.txt DEL response.txt
IF exist listdir.txt DEL listdir.txt
