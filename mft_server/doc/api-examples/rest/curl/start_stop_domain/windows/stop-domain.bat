...
set /p domain="Enter domain name to stop:"
...
:: Stop domain
for /F %%G in ('curl.exe -s -k -b cookie1.txt -w "%%{http_code}\n" -o NUL -H "x-js-at: !csrftoken1!" -H "X-Requested-With: XMLHttpRequest" -H "accept: application/json" -H "Content-type:application/json" -X POST %proto%://%jscapehost%:%jscapemanagmentport%/rest/management/domains/%domain%/stop
') do set stopDomainResponse=%%G
if !stopDomainResponse! EQU %server204% (echo Domain stopped successfully..) else (echo Invalid domain name or domain stop failed..)
) else (echo. 
