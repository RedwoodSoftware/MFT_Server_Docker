...
echo ##############################################################################
echo This example demonstrates how to delete a network storage using the server API.
echo ##############################################################################
...
set /p rName="Enter resource name:"
...
echo.
echo Login success. Proceeding to delete resource ...
echo.
...
for /F %%h in ('curl.exe -s -k -b cookie1.txt -w "%%{http_code}\n" -o NUL -H "x-js-at: !csrftoken1!" -H "X-Requested-With: XMLHttpRequest" -H "accept: application/json" -H "Content-type:application/json" -X GET %proto%://%jscapehost%:%jscapemanagmentport%/rest/management/network-storage/%domain%/%rName%
') do set IsResExistResponse=%%h

if !IsResExistResponse! EQU %server404% (echo Response code 404. Resource deletion failed .. Invalid Resource or domain name..) else (

:: Delete resource
for /F %%G in ('curl.exe -s -k -b cookie1.txt -w "%%{http_code}\n" -o NUL -H "x-js-at: !csrftoken1!" -H "X-Requested-With: XMLHttpRequest" -H "accept: application/json" -H "Content-type:application/json" -X DELETE %proto%://%jscapehost%:%jscapemanagmentport%/rest/management/network-storage/%domain%/%rName%
') do set delResResponse=%%G
if !delResResponse! EQU %server204% (echo Resource deleted successfully..)
if !delResResponse! EQU %server404% (echo Resource deletion failed..)
)) else (echo. 
